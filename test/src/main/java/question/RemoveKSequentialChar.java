		...
		for (int i = 1; i < inputArray.size(); i++) {
			if (start.equals(inputArray.get(i))) {
				...
				if (count >= burstLength) {
					...
					count = 1;
					if (!output.isEmpty() && start.equals(output.peek().getValue0())) {
						count= output.peek().getValue1()+count;
						Pair<String, Integer> pair = new Pair<String, Integer>(output.peek().getValue0(), count);
						output.pop();
						output.push(pair);
						continue;
					}
