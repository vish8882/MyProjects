...
public class VisitorController {
	...
	@RequestMapping("/feedbackdisplay.htm")
	public ModelAndView feedbackdisplay(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		if(response==null|| request==null)
		{
			log.info("Request or Response falied for UNREGISTEREVEN METHOD..");
			throw new FERSGenericException("Error in Transaction , Please re-Try for more information check Logfile in C:\\FERSLOG folder",
					new NullPointerException());
		}
	ArrayList<Object[]> visitor = new ArrayList<Object[]>();
		HttpSession hs=request.getSession();
		VisitorFacade serviceImpl = new VisitorServiceImpl();
		visitor=serviceImpl.feedbackdisplay();
		ModelAndView mv=new ModelAndView();
		mv.addObject("feedback",visitor);
		mv.setViewName("/feedbackdisplay.jsp");
		return mv;
	}
...
//System.out.println(regList.get(1));
		List<Object[]> eventList = new ArrayList<Object[]>();
	...
	public ModelAndView deleteaccount(HttpServletRequest request,
		...
		HttpSession session = request.getSession();
		Visitor visitor = (Visitor) session.getAttribute("VISITOR");
		int visitorid = Integer.parseInt(request.getParameter("vid"));
		VisitorFacade serviceImpl = new VisitorServiceImpl();
		int status = serviceImpl.deleteVisitor(visitorid);
		ModelAndView mv=new ModelAndView();
		if(status > 0)
		{
		mv.setViewName("/index.jsp");
	...
	@RequestMapping("/displayplaceasc.htm")
	public ModelAndView showEventsPlaceAsc(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		eventList = serviceImpl.getAllEventsPAsc();
	...
	@RequestMapping("/displayplacedesc.htm")
	public ModelAndView showEventsPlaceDesc(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		eventList = serviceImpl.getAllEventsPDesc();
	...
	public ModelAndView showEventsCatalogDesc(HttpServletRequest request,
		...
		System.out.println("id "+visitorid);
			VisitorFacade serviceImpl= new VisitorServiceImpl();
			status=serviceImpl.submitFeedback(feedback,vname,visitorid);
			System.out.println(status);
			ModelAndView mv=new ModelAndView();
			if(status==1)
			{
				System.out.println("yaha phch raha hai ry6hberhy6"+status);
				mv.addObject("fname",vname);
			mv.addObject("msg","Feedback Submitted");
	        mv.setViewName("/feedback.jsp");
			}
			else
			{
				mv.addObject("msg","Feedback Not Submitted");
				mv.setViewName("/feedback.jsp");
			}
